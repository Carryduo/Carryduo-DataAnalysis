...
taskProcess.on('message', function (m) {
    try {
        ...
        if (m.done === 'connect') {
            setTimeout(function () { taskProcess.send({ parameter: 1 }) }, 10000)
            console.log('부모프로세스에서 DB 연결 작업 완료 신호 받았다: ', process.cpuUsage(cpuUsage))
            return
        }
        if (m.done === 'collect') {
            console.log(m)
            setTimeout(function () { taskProcess.send({ parameter: m.parameter + 1 }) }, 10000)
        ...
        if (m.done === 'transfer') {
            console.log(m)
            m.parameter = 1
            setTimeout(function () { taskProcess.send({ parameter: m.parameter }) }, 10000)
